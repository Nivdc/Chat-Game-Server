...
function userJoinLobby(user){//fixme:大厅聊天室列表先加入的人看不到后面加入的人
    userList.push(user)
    lobbyChat.userJoin(user)
    sentLobbyInitInfo(user)
    lobbyChat.sentEventToAll("lobbyChatUserListUpd",lobbyChat.chatUserInfo())
...
class Room{
    ...
    startGame(){//todo
        lobbyListener.emit('gameStart',this.id)
        this.game.start()
    }
}

...
function userJoinRoom(cookieID,roomID){
    ...
    if(tempUser = userList.find(user => {return user.cookieID === cookieID})){
        if(!tempUser.currentRoom){//如果用户不在任何房间内
...
function kickUser(hostCookieID,roomID,userID){
    try{
        return roomList.find(room=>room.id===roomID).kickUser(hostCookieID,userID)
    }catch(error){
        return false
    }
}
...
class Room{
    ...
    roomInfo(){
        return {
            ...
            usersInfos:this.userList.map(user => user.info()),
        }
    }

    startGame(){
        if(this.userList.length <= this.roomGameInfo().maxPlayers || !this.roomGameInfo().maxPlayers){

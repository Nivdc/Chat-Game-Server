...
function init(){
    ...
    lobbyListener.on('gameStart',(room)=>{//todo
        room.status = "inGame"
    ...
    lobbyListener.on('gameOver',(room)=>{//todo
        room.status = "open"
        userList.forEach(user => {
            user.SSEconnection.write(`event:RoomInfoUpd\ndata:${JSON.stringify(room.roomInfo())}\n\n`)
            //xxx:只是更新个房间，咋要发送这么一大坨数据呢？但是好像很方便啊
...
function sentLobbyInitInfo(user){
    user.sentEvent("lobbyInit",{
...
function sentRoomChatMessage(message,senderCookieID,roomID){
    ...
    if(room = roomList.find(room => {return room.id === roomID})){
        return room.sentChatMessage(message,senderCookieID)
    }
    return false
}
...
function userJoinLobby(user){
    ...
    sentLobbyInitInfo(user)
    lobbyChat.sentEventToEveryone("lobbyChatUserListUpd",lobbyChat.chatUserInfo())
...
function startGame(roomID,userCookieID){
    let room = null
    if(room = roomList.find((room)=>{return room.id === roomID})){
        if(room.hostCookieID === userCookieID){
            room.startGame()
            return true
        }
    }
...
class Room{
    constructor(roomData,host,roomID){
        ...
        this.host=host
        this.id=roomID
        this.gameInfo = gameList.find(game => {return game.id === roomData.gameID})
    ...
    roomGameInfo(){//xxx:这里的信息可能多余了，只要有gameModeNum,一些信息可以在客户端读出来，但是自定义信息不能
        if(typeof(this.gameModeNum) === "number"){//如果这个值不是数字，说明使用的是自定义设置。
            return {
                gameName:this.gameInfo.gamePkgInfo.name,
                gameID:this.gameInfo.id,
                gameModeName:this.gameInfo.config.options[this.gameModeNum].modeName,
                maxPlayers:this.gameInfo.config.options[this.gameModeNum].maxPlayers
            }
        }
    ...
    startGame(){//todo
        if(this.userList.length <= this.roomGameInfo.maxPlayers){
            ...
            this.chat.sentEventToEveryone('gameStart')
            game = require(this.gameInfo.mainPath)
            if(typeof(this.gameModeNum) === "number"){//如果这个值不是数字，说明使用的是自定义设置。
                game.start(this.userList,this.this.gameInfo.config.options[this.gameModeNum],this.gameOver)
            }
            else if(this.customOption){
                game.start(this.userList,this.customOption,this.gameOver)
            }
    ...
    gameOver(){
        ...
        this.chat.sentEventToEveryone('gameOver')
    }
}
